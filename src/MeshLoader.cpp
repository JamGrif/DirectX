...
MeshLoader::MeshLoader()
	:fbuffer(), fbuffersize(0), actualsize(0), tokenptr(0), filenameString("")
{
	//Ensure any arrays and vectors are empty before use
	delete[] vertices;
...
MeshLoader::~MeshLoader()
{
	//Clear up any arrays and vectors
	delete[] vertices;
...
void MeshLoader::LoadObj(char* fname, ID3D11Buffer*& vb, unsigned int& numverts)
{
	//Delete existing loaded model
	if (vb) vb->Release();

	if (loadfile(fname) == 0)
	{
		// file not loaded, check debug output;
		filenameString = "FILE NOT LOADED";
		return;
	}
...
int MeshLoader::loadfile(char* fname)
{
	FILE* pFile;
...
void MeshLoader::parsefile(char* filename)
{
	...
	do
	{
		...
		if (strncmp(&fbuffer[tokenstart], "v ", 2) == 0) // VERTEX POSITION - note the space in the string is needed (see vt, etc)
		{
			...
			if (!success) { char s[100] = "ERROR: Badly formatted vertex, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filenameString.c_str());  DXTRACE_MSG(s); }
		...
		else if (strncmp(&fbuffer[tokenstart], "vt", 2) == 0) // TEXTURE COORDINATES
		{
			...
			if (!success) { char s[100] = "ERROR: Badly formatted texture coordinate, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filenameString.c_str());  DXTRACE_MSG(s); }
		...
		else if (strncmp(&fbuffer[tokenstart], "vn", 2) == 0)  // NORMALS
		{
			...
			if (!success) { char s[100] = "ERROR: Badly formatted normal, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filenameString.c_str());  DXTRACE_MSG(s); }
		...
		else if (strncmp(&fbuffer[tokenstart], "f ", 2) == 0)  // FACE - only deals with triangles so far
		{
			...
			if (!success) { char s[100] = "ERROR: Badly formatted face, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filenameString.c_str());  DXTRACE_MSG(s); }
		}
...
bool MeshLoader::getnexttoken(int& tokenstart, int& tokenlength)
{
	tokenstart = tokenptr;
	tokenlength = 1;
	int tokenend;
...
bool MeshLoader::createVB(ID3D11Buffer*& vb, unsigned int& numverts)
{
	...
	bufferDesc.ByteWidth = sizeof(MODEL_POS_TEX_NORM_VERTEX) * numverts;	//Size of vertex buffer to be created
	bufferDesc.Usage = D3D11_USAGE_IMMUTABLE;								//How buffer will be used (D3D11_USAGE type) Immutable = contents never change after creation							
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;						//Use as vertex buffer
	bufferDesc.CPUAccessFlags = 0;											//How CPU will access buffer - 0 means no access
	bufferDesc.MiscFlags = 0;												//No misc flags for vertex buffer
	bufferDesc.StructureByteStride = 0;										//Only applies for structured buffers 

	D3D11_SUBRESOURCE_DATA VBinitdata;										//Data to initalize the buffer with
	ZeroMemory(&VBinitdata, sizeof(VBinitdata));
	VBinitdata.pSysMem = vertices;											//Array that contains data to initialize vertex buffer

	HR(EngineStatics::GetD3DDevice()->CreateBuffer(&bufferDesc, &VBinitdata, &vb));	// Create the buffer




	// REQUIRED IF D3D11_USAGE_DYNAMIC FOR bufferDesc.Usage
	//Copy the vertices into the buffer
	//D3D11_MAPPED_SUBRESOURCE ms;
	//EngineStatics::GetD3DContext()->Map(pVertexBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &ms);	// Lock the buffer to allow writing
	//memcpy(ms.pData, vertices, sizeof(vertices[0]) * numverts);							// Copy the data
	//EngineStatics::GetD3DContext()->Unmap(pVertexBuffer, NULL);										// Unlock the buffer
